...
class GNNModel(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        layers = [geom_nn.GINEConv(gnn_layer(in_channels=in_channels, out_channels=out_channels, **kwargs),edge_dim=edge_dim)] 
        in_channels = c_hidden
        for l_idx in range(num_layers):
            layers += [
                geom_nn.GINEConv(gnn_layer(in_channels=in_channels, out_channels=out_channels, **kwargs),edge_dim=edge_dim),
#                nn.ReLU(inplace=True),
#                 nn.Dropout(dp_rate),
            ]
#            in_channels = c_hidden
        layers += [geom_nn.GINEConv(gnn_layer(in_channels=in_channels, out_channels=c_out, **kwargs),edge_dim=edge_dim)]
...
#     gnn.PrintOvito()
    ...
    for epoch in range( gnn.num_epochs ):
        optimizer.zero_grad()
        predicted_displacements = gnn(gnn.dataset_train.x.to(device), gnn.dataset_train.edge_index.to(device),gnn.dataset_train.edge_attr.to(device))
        training_loss              = criterion(predicted_displacements, gnn.dataset_train.y.to(device))
        training_loss.backward()
        optimizer.step()
        if epoch == 0:
        ...
        gnn.eval()
        with torch.no_grad():  # Disable gradient calculation
                predicted_displacements = gnn(gnn.dataset_test.x.to(device), gnn.dataset_test.edge_index.to(device), gnn.dataset_test.edge_attr.to(device))
                validation_loss         = criterion(predicted_displacements, gnn.dataset_test.y.to(device))
